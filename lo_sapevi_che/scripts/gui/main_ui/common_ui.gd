...
func fade(node : Node, final_node : float, final_gui : float) -> void:
	var tween = create_tween()
	tween.set_parallel()
	tween.tween_property(node, "modulate:a", final_node, 0.8).set_ease(Tween.EASE_OUT)
	if self != node: # if self and node are the same node, this animation shouldnt be called
		tween.tween_property(self, "modulate:a", final_gui, 0.8).set_ease(Tween.EASE_OUT)
	await tween.finished

func fade_in(node : Node) -> void:
	await fade(node, 1.2, 0.2)

func fade_out(node : Node) -> void:
	await fade(node, 0.0, 1.0)
