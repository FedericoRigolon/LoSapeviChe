...
func start(delay: float = 0, is_entry: bool = true):
	...
	(
		tween
		. tween_property(current_node, "position:x", end_pos_x, self.duration)
		. set_trans(Tween.TRANS_QUAD if is_entry else Tween.TRANS_CUBIC)
		. set_ease(Tween.EASE_OUT if is_entry else Tween.EASE_IN)
	)
	(
		tween
		. parallel()
		. tween_property(current_node, "modulate:a", end_opacity, self.duration)
